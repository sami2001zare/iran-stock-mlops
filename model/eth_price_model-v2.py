...
MAX_BATCHES      = 50           # Maximum number of batches
...
# Save the pruned model (best performing)
joblib.dump(model_pruned, os.path.join(MODEL_DIR, "model_pruned.pkl"))

# Save the QuantileTransformer (must be applied to new data before prediction)
# In simple terms, this code saves the "memory and mathematical formulas" related to the normalization of your data onto the hard disk, so that you can use them later without needing to retrain.
joblib.dump(qt, os.path.join(MODEL_DIR, "quantile_transformer.pkl"))

# Save pipeline metadata (feature names, pruning mask, etc.)
pipeline_meta = {
    "feature_cols":        FEATURE_COLS,
    "pruned_cols":         PRUNED_COLS,
    "mask_keep":           mask_keep.tolist(),
    "windows":             [5, 10, 20, 50, 100],        # Rolling windows used for moving averages and other features
    "lag_steps":           [1, 2, 3, 5, 10, 20],        # Lag steps used for price of previous days
